...
#define ALLOC_HEAP(n) do { cells = mhs_mmalloc(n * sizeof(node)); } while(0)
...
{
  struct ioarray *arr = mhs_mmalloc(sizeof(struct ioarray) + (sz-1) * sizeof(NODEPTR));
  size_t i;
...
add_tick_table(struct bytestring name)
{
  if (!tick_table) {
    tick_table_size = 100;
    tick_table = mhs_mmalloc(tick_table_size * sizeof(struct tick_entry));
    tick_index = 0;
  }
  if (tick_index >= tick_table_size) {
    tick_table_size *= 2;
    tick_table = mhs_mrealloc(tick_table, tick_table_size * sizeof(struct tick_entry));
  }
...
init_stableptr(void)
{
  sp_table = mhs_mmalloc(sp_capacity * sizeof(NODEPTR)); /* stable pointer table, all free */
...
new_stableptr(NODEPTR n)
{
  ...
  if (i == sp_capacity) {
    ...
    sp_capacity *= 2;
    sp_table = mhs_mrealloc(sp_table, sp_capacity * sizeof(NODEPTR));
...
{
  struct mthread *mt = mhs_mmalloc(sizeof(struct mthread));
...
new_mvar(void)
{
  ...
  struct mvar *mv = mhs_mmalloc(sizeof(struct mvar));
...
{
  struct final *fin = mhs_mcalloc(1, sizeof(struct final));
  struct forptr *fp = mhs_mcalloc(1, sizeof(struct forptr));
...
init_nodes(void)
{
  ...
  free_map = mhs_mmalloc(free_map_nwords * sizeof(bits_t));
...
{
  struct weak_ptr *wp = mhs_mmalloc(sizeof(struct weak_ptr));
  wp->next = allweaks;
  allweaks = wp;
  wp->marked = 0;
  wp->key = key;
...
parse_string(BFILE *f)
{
  ...
  buffer = mhs_mrealloc(buffer, i + 1);
...
parse_top(BFILE *f, struct ffe_entry *ffe)
{
  ...
  shared_table = mhs_mmalloc(shared_table_size * sizeof(struct shared_entry));
...
printb(BFILE *f, NODEPTR n, bool header)
{
  ...
  num_shared = 0;
  pb.marked_bits = mhs_mcalloc(free_map_nwords, sizeof(bits_t));
  pb.shared_bits = mhs_mcalloc(free_map_nwords, sizeof(bits_t));
  find_sharing(&pb, n);
  if (header) {
    putsb(VERSION, f);
...
{
  struct forptr *fp = mhs_mmalloc(sizeof(struct forptr));
...
rnf(value_t noerr, NODEPTR n)
{
  ...
  bits_t *done = mhs_mcalloc(free_map_nwords, sizeof(bits_t));
  if (doing_rnf)
    ERR("recursive rnf()");
  ...
  case T_CATCHR:
    {
      ...
      struct handler *h = mhs_mmalloc(sizeof *h);
...
)
{
  ...
  init_nodes();
  stack = mhs_mmalloc(sizeof(NODEPTR) * stack_size);
  CLEARSTK();
  init_stableptr();
...
#endif
  mpz_ptr p = mhs_mmalloc(sizeof(*p));
  mpz_init(p);
