...
/* midi_file.h is optional - only for MIDI file recording/reading */
#ifdef MHS_MIDI_FILE_SUPPORT
#include "midi_file.h"
#endif
...
/* SharedContext override for editor integration.
 * When set, MIDI events are routed through SharedContext instead of
 * going directly to libremidi. This enables TSF/Csound/Link integration. */
#ifdef PSND_SHARED_CONTEXT
#include "context.h"
static struct SharedContext *g_shared_override = NULL;

void midi_ffi_set_shared(struct SharedContext *ctx) {
    g_shared_override = ctx;
}
#else
/* Stub for standalone builds */
void midi_ffi_set_shared(void *ctx) {
    (void)ctx;
}
#endif
...
#ifdef PSND_SHARED_CONTEXT
    /* Route through SharedContext if available (editor mode) */
    if (g_shared_override) {
        shared_send_note_on(g_shared_override, channel, pitch, velocity);
        capture_add_event(0, channel - 1, pitch, velocity);
        return 0;
    }
#endif

    /* Direct MIDI output (standalone mode) */
...
int midi_note_off(int channel, int pitch) {
    ...
    /* Route through SharedContext if available (editor mode) */
    if (g_shared_override) {
        shared_send_note_off(g_shared_override, channel, pitch);
...
int midi_cc(int channel, int controller, int value) {
    ...
    /* Route through SharedContext if available (editor mode) */
    if (g_shared_override) {
        shared_send_cc(g_shared_override, channel, controller, value);
...
int midi_program(int channel, int program) {
    ...
    /* Route through SharedContext if available (editor mode) */
    if (g_shared_override) {
        shared_send_program(g_shared_override, channel, program);
        return 0;
    }
#endif
...
void midi_panic(void) {
    ...
    /* Route through SharedContext if available (editor mode) */
    if (g_shared_override) {
        shared_send_panic(g_shared_override);
        return;
    }
#endif
...
int midi_save_mid(const char* filename) {
    (void)filename;
    printf("MIDI file support not available\n");
    return -1;
}
#endif
